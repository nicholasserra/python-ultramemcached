...
"""
client module for memcached (memory cache daemon) using ultramemcached
...
__author__    = "Nicholas Serra <nick@528hazelwood.com>"
__version__ = "0.0.1"
...
class Client(local):
    ...
    def delete_multi(self, keys, time=0, key_prefix=''):
        ...
        UMEMCACHE DOES NOT SUPPORT DELETE_MULTI, this function simply loops
        over a standard del, and is convenience only until it is supported.
    ...
    def delete(self, key, time=0):
        ...
        try:
            if time != None and time != 0:
                response = server.delete(key, time)
            else:
                response = server.delete(key)
            if response in ['DELETED', 'NOT_FOUND']: return 1
    ...
    def _incrdecr(self, cmd, key, delta):
        ...
        try:
            response = getattr(server, cmd)(key, delta)
            if response == 'NOT_FOUND' or response == '0':
                return None
            else:
                return int(response)
        except socket.error, msg:
    ...
    def set_multi(self, mapping, time=0, key_prefix='', min_compress_len=0):
        ...
        UMEMCACHE DOES NOT SUPPORT SET_MULTI, this function simply loops
        over a standard set, and is convenience only until it is supported.
            ...
            try:
                for key in server_keys[server]: # These are mangled keys
                    store_info = self._val_to_store_info(
                            ...
                            min_compress_len)
                    if store_info:
                        response = server.set(key, store_info[2], time, store_info[0])
                        if response == "STORED":
                            continue
    ...
    def _set(self, cmd, key, val, time, min_compress_len = 0):
        ...
        def _unsafe_set():
            ...
            try:
                response = getattr(server, cmd)(*args)
        ...
        try:
            ...
            try:
                return _unsafe_set()
            except (_ConnectionDeadError), msg:
                server.mark_dead(msg)
            return 0
    ...
    def _get(self, cmd, key):
        ...
        def _unsafe_get():
            self._statlog(cmd)
            try:
                response = getattr(server, cmd)(key)
    ...
    def get_multi(self, keys, key_prefix=''):
        ...
        for server in server_keys.iterkeys():
            try:
                response = server.get_multi(server_keys[server])
                for k in response:
                    value, flags = response[k]
                    val = self._recv_value(value, flags)
                    retvals[prefixed_to_orig_key[k]] = val   # un-prefix returned key.
...
class _Host(object):
    ...
    def set(self, m, key, val, timeout, flags):
        try:
            response = m.set(key, val, timeout, flags)
        except RuntimeError:
